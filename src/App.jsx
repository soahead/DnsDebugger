...
async function checkDNSBL(ip, zone) {
  try {
    ...
    if (answers.length > 0) return { listed: true, response: answers[0].data };
    return { listed: false };
  } catch(e) {
    return { listed: null };
  }
...
async function checkPTR(ip) {
  try {
    ...
    if (answers.length > 0) return { found: true, hostname: answers[0].data.replace(/\.$/, "") };
...
async function checkDKIM(domain) {
  ...
  try {
    for (const sel of selectors) {
      ...
      if (dkimRec) return { found: true, selector: sel, value: dkimRec.data.replace(/"/g, "").trim() };
    }
    return { found: false };
  } catch(e) {
    return { found: null };
...
async function queryNS(domain) {
  try {
    ...
    return answers.map(r => ({ ttl: r.TTL, data: r.data.replace(/\.$/, "").trim() }));
  } catch(e) {
    return [];
  }
}

// ── NEW: SMTP banner check via public API ───────────────────────────────────
// Uses mxtoolbox-style public check — we do a best-effort using a CORS-friendly
// SMTP probe. Since browsers can't open raw TCP, we resolve MX then flag for
// manual verification and provide the mxtoolbox deep-link.
async function checkSMTPBanner(mxHost, domain) {
  // We can't do raw TCP from a browser, so we:
  // 1. Resolve the MX host to an IP
  // 2. Check if the hostname contains the sending domain (basic heuristic)
  // 3. Return a structured result with a mxtoolbox link for full check
  try {
    const url = "https://dns.google/resolve?name=" + encodeURIComponent(mxHost) + "&type=A";
    const res = await fetch(url, { headers: { Accept: "application/dns-json" } });
    const data = await res.json();
    const answers = (data.Answer || []).filter(r => r.type === 1);
    const ip = answers.length > 0 ? answers[0].data : null;

    // Heuristic: banner SHOULD contain the MX hostname or domain
    // We flag if the MX host looks generic (e.g. shared hosting patterns)
    const looksHosted = mxHost.includes("hostedemail.com") || mxHost.includes("exacthosting");
    const mxtoolboxUrl = "https://mxtoolbox.com/SuperTool.aspx?action=smtp%3a" + encodeURIComponent(mxHost) + "&run=toolpage";
    ...
    return {
      ...
      mxtoolboxUrl,
      note: ip
        ? "Resolved to " + ip + ". Full banner verification requires TCP access."
        : "Could not resolve MX host to an IP.",
    };
  } catch(e) {
    return { mxHost, ip: null, looksHosted: false, mxtoolboxUrl: null, note: "Check failed." };
  }
}

// ── NEW: DMARC aggregate report endpoint validator ──────────────────────────
function parseDMARCRua(dmarcVal) {
  if (!dmarcVal) return [];
  const match = dmarcVal.match(/rua=([^;]+)/i);
  if (!match) return [];
  return match[1].split(",").map(s => s.trim()).filter(Boolean);
}

async function validateRuaEndpoint(ruaUri) {
  // mailto: — resolve MX of the target domain and confirm it exists
  if (ruaUri.startsWith("mailto:")) {
    const email = ruaUri.replace("mailto:", "");
    const parts = email.split("@");
    if (parts.length !== 2) return { uri: ruaUri, status: "invalid", detail: "Malformed email address." };
    const emailDomain = parts[1];

    // Check for external destination authorization (_report._dmarc.<external-domain>)
    // Only needed if the rua domain ≠ the sending domain — caller handles that check
    try {
      const url = "https://dns.google/resolve?name=" + encodeURIComponent(emailDomain) + "&type=MX";
      const res = await fetch(url, { headers: { Accept: "application/dns-json" } });
      const data = await res.json();
      const answers = (data.Answer || []).filter(r => r.type === 15);
      if (answers.length > 0) {
        return { uri: ruaUri, status: "ok", detail: "MX found for " + emailDomain + " (" + answers[0].data.split(" ").slice(1).join("") + ")" };
      } else {
        return { uri: ruaUri, status: "warn", detail: "No MX record found for " + emailDomain + " — reports may not be delivered." };
      }
    } catch(e) {
      return { uri: ruaUri, status: "error", detail: "DNS lookup failed for " + emailDomain };
    }
  }

  // https: endpoint
  if (ruaUri.startsWith("https://")) {
    return { uri: ruaUri, status: "info", detail: "HTTPS report endpoint — reachability cannot be verified from a browser." };
  }

  return { uri: ruaUri, status: "warn", detail: "Unrecognised rua URI scheme." };
}

async function validateAllRua(dmarcVal, sendingDomain) {
  const uris = parseDMARCRua(dmarcVal);
  if (uris.length === 0) return { uris: [], missing: true };

  const results = await Promise.all(uris.map(async uri => {
    const result = await validateRuaEndpoint(uri);
    // Check external authorization if needed
    if (uri.startsWith("mailto:")) {
      const emailDomain = uri.replace("mailto:", "").split("@")[1];
      if (emailDomain && emailDomain.toLowerCase() !== sendingDomain.toLowerCase()) {
        // Need _report._dmarc.<emailDomain> TXT to authorize cross-domain reporting
        try {
          const authQuery = "_report._dmarc." + emailDomain;
          const url = "https://dns.google/resolve?name=" + encodeURIComponent(authQuery) + "&type=TXT";
          const res = await fetch(url, { headers: { Accept: "application/dns-json" } });
          const data = await res.json();
          const answers = (data.Answer || []).filter(r => r.type === 16);
          result.externalAuth = answers.length > 0 ? "authorized" : "missing";
          result.externalDomain = emailDomain;
        } catch(e) {
          result.externalAuth = "error";
          result.externalDomain = emailDomain;
        }
      }
    }
  ...
  return { uris: results, missing: false };
}
...
// ── Export helpers ──────────────────────────────────────────────────────────
function buildShareableLink(domain) {
  const base = window.location.href.split("?")[0];
  return base + "?domain=" + encodeURIComponent(domain);
}

function exportTextReport(result) {
  if (!result) return "";
  const lines = [];
  lines.push("DNS DEBUGGER REPORT");
  lines.push("===================");
  lines.push("Domain  : " + result.domain);
  lines.push("Scanned : " + new Date().toUTCString());
  ...
  result.flags.forEach(f => {
    lines.push("[" + (f.sev === "ok" ? "PASS" : f.sev.toUpperCase()) + "] " + f.title);
  ...
  if (result.ns && result.ns.length > 0) {
    result.ns.forEach(r => lines.push("  " + r.data + " (TTL " + r.ttl + "s)"));
  } else {
  ...
  lines.push("-----------");
  ["A","AAAA","MX","TXT","spf","dmarc"].forEach(type => {
    const recs = result.records[type];
    if (recs && recs.length > 0) {
      recs.forEach(r => lines.push("  " + type.toUpperCase() + " " + r.data));
    }
  });
  lines.push("");
  lines.push("Generated by Exact Hosting DNS Debugger");
  return lines.join("\n");
}
...
function downloadTextReport(result) {
  ...
  a.href     = url;
  a.download = result.domain + "-dns-report.txt";
  a.click();
...
const STYLE = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    ...
    --purple: #5243aa; --purple-bg: #f3f0ff; --purple-border: #c0b6f2;
    --teal: #008080; --teal-bg: #e0f5f5; --teal-border: #9de0e0;
    --accent: #0052cc;
  ...
  .btn-toolbar { display: inline-flex; align-items: center; gap: 5px; background: var(--white); color: var(--text-mid); border: 1px solid var(--border2); border-radius: 5px; font-family: var(--mono); font-size: 10px; font-weight: 600; letter-spacing: 0.08em; padding: 5px 12px; cursor: pointer; transition: all 0.15s; white-space: nowrap; text-transform: uppercase; text-decoration: none; }
  .btn-toolbar:hover { background: var(--blue-bg); color: var(--accent); border-color: var(--blue-border); }
  .btn-toolbar.copied { background: var(--green-bg); color: var(--green); border-color: var(--green-border); }
  .toolbar-sep { width: 1px; height: 20px; background: var(--border2); }
  .share-link-box { display: flex; align-items: center; gap: 8px; background: var(--bg); border: 1px solid var(--border); border-radius: 5px; padding: 6px 10px; font-family: var(--mono); font-size: 10px; color: var(--text-dim); flex: 1; min-width: 0; }
  .share-link-val { flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text-mid); }
  ...
  /* NS Panel */
  .ns-row { display: grid; grid-template-columns: 1fr 60px; gap: 10px; padding: 9px 14px; border-bottom: 1px solid var(--border); font-family: var(--mono); font-size: 11px; align-items: center; }
  .ns-row:last-child { border-bottom: none; }
  .ns-row:hover { background: var(--bg); }
  .ns-host { color: var(--text); }
  .ns-ttl  { color: var(--text-dim); text-align: right; }
  .ns-mismatch-notice { padding: 9px 14px; background: var(--amber-bg); border-top: 1px solid var(--amber-border); font-size: 11px; color: var(--amber); font-family: var(--mono); }

  /* SMTP panel */
  .smtp-panel { padding: 12px 14px; background: var(--bg); border-top: 1px solid var(--border); }
  .smtp-title { font-size: 10px; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; color: var(--text-dim); font-family: var(--mono); margin-bottom: 10px; }
  .smtp-row { display: flex; flex-direction: column; gap: 3px; padding: 8px 0; border-bottom: 1px solid var(--border); }
  .smtp-row:last-child { border-bottom: none; }
  .smtp-host { font-family: var(--mono); font-size: 12px; font-weight: 500; color: var(--text); }
  .smtp-ip   { font-family: var(--mono); font-size: 10px; color: var(--text-dim); }
  .smtp-note { font-size: 11px; color: var(--text-mid); }
  .smtp-actions { display: flex; gap: 6px; margin-top: 4px; align-items: center; flex-wrap: wrap; }
  .btn-smtp-ext { display: inline-flex; align-items: center; gap: 3px; background: var(--white); color: var(--accent); border: 1px solid var(--blue-border); border-radius: 3px; font-family: var(--mono); font-size: 9px; font-weight: 700; letter-spacing: 0.08em; padding: 2px 8px; text-decoration: none; text-transform: uppercase; white-space: nowrap; transition: all 0.15s; }
  .btn-smtp-ext:hover { background: var(--blue-bg); border-color: var(--accent); }
  .smtp-banner-note { margin: 10px 14px; padding: 9px 12px; background: var(--blue-bg); border: 1px solid var(--blue-border); border-radius: 6px; font-size: 11px; color: var(--blue); font-family: var(--mono); line-height: 1.5; }

  /* DMARC RUA panel */
  .dmarc-panel { padding: 12px 14px; border-top: 1px solid var(--border); background: var(--bg); }
  .dmarc-panel-title { font-size: 10px; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; color: var(--text-dim); font-family: var(--mono); margin-bottom: 10px; }
  .rua-row { display: flex; flex-direction: column; gap: 3px; padding: 8px 0; border-bottom: 1px solid var(--border); font-size: 12px; }
  .rua-row:last-child { border-bottom: none; }
  .rua-uri { font-family: var(--mono); font-size: 11px; color: var(--text); word-break: break-all; }
  .rua-detail { font-size: 10px; color: var(--text-mid); font-family: var(--mono); }
  .rua-ext-warn { font-size: 10px; background: var(--amber-bg); color: var(--amber); border: 1px solid var(--amber-border); border-radius: 3px; padding: 2px 6px; display: inline-block; margin-top: 2px; font-family: var(--mono); }
  .rua-ext-ok { font-size: 10px; background: var(--green-bg); color: var(--green); border: 1px solid var(--green-border); border-radius: 3px; padding: 2px 6px; display: inline-block; margin-top: 2px; font-family: var(--mono); }
  .rua-status-ok   { font-size: 9px; font-weight: 700; letter-spacing: 0.1em; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; font-family: var(--mono); background: var(--green-bg); color: var(--green); border: 1px solid var(--green-border); display: inline-block; }
  .rua-status-warn { font-size: 9px; font-weight: 700; letter-spacing: 0.1em; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; font-family: var(--mono); background: var(--amber-bg); color: var(--amber); border: 1px solid var(--amber-border); display: inline-block; }
  .rua-status-info { font-size: 9px; font-weight: 700; letter-spacing: 0.1em; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; font-family: var(--mono); background: var(--blue-bg); color: var(--blue); border: 1px solid var(--blue-border); display: inline-block; }
  .rua-status-error{ font-size: 9px; font-weight: 700; letter-spacing: 0.1em; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; font-family: var(--mono); background: var(--red-bg); color: var(--red); border: 1px solid var(--red-border); display: inline-block; }
  .dmarc-rua-checking { font-size: 11px; color: var(--text-dim); font-family: var(--mono); padding: 10px 0; }
  .btn-rua { background: var(--white); color: var(--purple); border: 1px solid var(--purple-border); border-radius: 4px; font-family: var(--mono); font-size: 10px; font-weight: 600; letter-spacing: 0.08em; padding: 4px 10px; cursor: pointer; transition: all 0.15s; white-space: nowrap; text-transform: uppercase; }
  .btn-rua:hover:not(:disabled) { background: var(--purple-bg); }
  .btn-rua:disabled { opacity: 0.5; cursor: not-allowed; }
...
function NSSection({ ns }) {
  if (!ns || ns.length === 0) {
    return (
      <div className="flag">
        <span className="flag-icon" style={{ color: "var(--red)" }}>FAIL</span>
        <div className="flag-body">
          <div className="flag-title">No NS records found</div>
          <span className="sev-tag sev-critical">critical</span>
        </div>
      </div>
    );
  }

  // Detect nameserver provider from hostnames
  const providers = new Set();
  ns.forEach(r => {
    const h = r.data.toLowerCase();
    if (h.includes("exacthosting") || h.includes("exactdns")) providers.add("Exact Hosting");
    else if (h.includes("cloudflare")) providers.add("Cloudflare");
    else if (h.includes("awsdns") || h.includes("amazonaws")) providers.add("AWS Route 53");
    else if (h.includes("google")) providers.add("Google Cloud DNS");
    else if (h.includes("godaddy") || h.includes("domaincontrol")) providers.add("GoDaddy");
    else providers.add("Unknown / Third-party");
  });

  // Check all NS are on the same provider (mismatch = warning)
  const mismatch = providers.size > 1;

  return (
    <div>
      {ns.map((r, i) => (
        <div className="ns-row" key={i}>
          <span className="ns-host">{r.data}</span>
          <span className="ns-ttl">{ttlToHuman(r.ttl)}</span>
        </div>
      ))}
      {mismatch && (
        <div className="ns-mismatch-notice">
          Warning: Nameservers appear to be from multiple providers ({[...providers].join(", ")}). Verify this is intentional.
        </div>
      )}
      {!mismatch && (
        <div className="ttl-note">
          Provider detected: {[...providers].join(", ")}
        </div>
      )}
    </div>
  );
...
// ── NEW: SMTP Banner Section ────────────────────────────────────────────────
function SMTPSection({ mxRecords, domain }) {
  const [smtpState, setSmtpState] = useState("idle");
  const [smtpResults, setSmtpResults] = useState([]);

  async function runSmtpChecks() {
    setSmtpState("checking");
    const hosts = mxRecords.map(r => parseMXHost(r.data)).filter(Boolean);
    const results = await Promise.all(hosts.map(h => checkSMTPBanner(h, domain)));
    setSmtpResults(results);
    setSmtpState("done");
  }

  if (!mxRecords || mxRecords.length === 0) {
    return (
      <div style={{ padding: "12px 14px", fontSize: 11, color: "var(--text-dim)", fontFamily: "var(--mono)" }}>
        No MX records to check — run a domain scan first.
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12, background: "var(--white)" }}>
        <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-dim)" }}>
          {mxRecords.length} MX host{mxRecords.length !== 1 ? "s" : ""} detected
        </span>
        <button
          className="btn-blacklist"
          onClick={runSmtpChecks}
          disabled={smtpState === "checking"}
          style={{ color: "var(--teal)", borderColor: "var(--teal-border)" }}
        >
          {smtpState === "checking" ? "Checking..." : smtpState === "done" ? "Re-check" : "Check SMTP Hosts"}
        </button>
      </div>

      {smtpState === "done" && (
        <div className="smtp-panel">
          <div className="smtp-title">SMTP Host Resolution Results</div>
          {smtpResults.map((r, i) => (
            <div className="smtp-row" key={i}>
              <div className="smtp-host">{r.mxHost}</div>
              <div className="smtp-ip">{r.ip ? "Resolves to: " + r.ip : "Could not resolve to IP"}</div>
              <div className="smtp-note">{r.note}</div>
              <div className="smtp-actions">
                {r.mxtoolboxUrl && (
                  <a className="btn-smtp-ext" href={r.mxtoolboxUrl} target="_blank" rel="noopener noreferrer">
                    Full Banner Check (MXToolbox) ↗
                  </a>
                )}
                {r.ip && (
                  <a
                    className="btn-smtp-ext"
                    href={"https://mxtoolbox.com/SuperTool.aspx?action=smtp%3a" + encodeURIComponent(r.ip) + "&run=toolpage"}
                    target="_blank"
          ...
          <div className="smtp-banner-note">
            i  Full SMTP banner inspection (HELO/EHLO verification) requires a direct TCP connection on port 25, which browsers cannot open. Use the MXToolbox links above to complete the banner check for each host. The check above confirms the MX hostname resolves correctly and pre-fills the external tool.
          </div>
        </div>
      )}
    </div>
...
// ── NEW: DMARC RUA Validator ────────────────────────────────────────────────
function DMARCRuaSection({ dmarcVal, domain }) {
  const [ruaState, setRuaState] = useState("idle");
  const [ruaData, setRuaData] = useState(null);

  const uris = parseDMARCRua(dmarcVal);

  async function runRuaCheck() {
    setRuaState("checking");
    const data = await validateAllRua(dmarcVal, domain);
    setRuaData(data);
    setRuaState("done");
  }
  ...
  if (!dmarcVal) {
    return (
      <div style={{ padding: "12px 14px", fontSize: 11, color: "var(--text-dim)", fontFamily: "var(--mono)" }}>
        No DMARC record found — scan a domain first.
      </div>
    );
  }
  ...
  if (uris.length === 0) {
    return (
      <div style={{ padding: "12px 14px" }}>
        <div className="flag">
          <span className="flag-icon" style={{ color: "var(--amber)" }}>WARN</span>
          <div className="flag-body">
            <div className="flag-title">No rua= tag in DMARC record</div>
            <div className="flag-detail">{dmarcVal}</div>
            <div className="flag-fix">Fix: Add rua=mailto:dmarc-reports@{domain} to receive aggregate reports.</div>
            <span className="sev-tag sev-warning">warning</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12, background: "var(--white)", flexWrap: "wrap" }}>
        <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-dim)" }}>
          {uris.length} rua endpoint{uris.length !== 1 ? "s" : ""} found
        </span>
        <button className="btn-rua" onClick={runRuaCheck} disabled={ruaState === "checking"}>
          {ruaState === "checking" ? "Validating..." : ruaState === "done" ? "Re-validate" : "Validate rua Endpoints"}
        </button>
      </div>
      ...
      {ruaState === "idle" && (
        <div style={{ padding: "10px 14px" }}>
          {uris.map((u, i) => (
            <div key={i} style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-mid)", padding: "4px 0" }}>{u}</div>
          ))}
        </div>
      )}

      {ruaState === "checking" && (
        <div className="dmarc-panel">
          <div className="dmarc-rua-checking">Validating DMARC aggregate report endpoints...</div>
        </div>
      )}

      {ruaState === "done" && ruaData && (
        <div className="dmarc-panel">
          <div className="dmarc-panel-title">rua Endpoint Validation</div>
          {ruaData.missing ? (
            <div style={{ fontSize: 11, color: "var(--amber)", fontFamily: "var(--mono)" }}>No rua endpoints configured.</div>
          ) : (
            ruaData.uris.map((r, i) => (
              <div className="rua-row" key={i}>
                <div className="rua-uri">{r.uri}</div>
                <div className="rua-detail">{r.detail}</div>
                {r.externalAuth === "missing" && (
                  <span className="rua-ext-warn">
                    External domain — missing _report._dmarc.{r.externalDomain} authorization record
                  </span>
                )}
                {r.externalAuth === "authorized" && (
                  <span className="rua-ext-ok">
                    External domain — authorization confirmed (_report._dmarc.{r.externalDomain})
                  </span>
                )}
                <span className={"rua-status-" + r.status}>{r.status}</span>
              </div>
            ))
          )}
        </div>
...
function MicrosoftPanel({ ip, domain, spfVal, dmarcVal }) {
  ...
  async function runMsChecks() {
    ...
    if (dkim.found === null) {
      ...
      checks.push({ sev: "fail", title: "No DKIM record found", detail: "Checked selectors: default, google, mail, dkim, k1, selector1, selector2", fix: "Set up DKIM signing for " + domain + ". Microsoft filters weight DKIM heavily." });
    } else {
          ...
          <div className="ms-jmrp">
            ...
            <div className="ms-jmrp-body">
              ...
              <div className="ms-jmrp-text">
                Check whether this IP is enrolled in Microsoft's Junk Mail Reporting Program (JMRP).{" "}
            ...
            <div className="bl-row" key={i}>
              ...
              <div className="bl-actions">
                ...
                {r.listed === true && r.delistUrl && (
                  <a className="btn-delist" href={r.delistUrl} target="_blank" rel="noopener noreferrer">Delist ↗</a>
                )}
              </div>
            </div>
          ))}
...
function ExportToolbar({ result }) {
  ...
  function handleCopyLink() {
    const link = buildShareableLink(result.domain);
  ...
  const shareLink = buildShareableLink(result.domain);

  return (
    <div className="toolbar">
      <button className={"btn-toolbar" + (copied ? " copied" : "")} onClick={handleCopyReport}>
      ...
      <button className={"btn-toolbar" + (linkCopied ? " copied" : "")} onClick={handleCopyLink}>
        {linkCopied ? "✓ Link Copied!" : "Copy Share Link"}
      </button>
      <div className="share-link-box">
        <span className="share-link-val">{shareLink}</span>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (d) {
        setDomain(d);
        // Auto-scan after a tick so state is set
        setTimeout(() => {
          document.getElementById("btn-scan-main")?.click();
        }, 100);
      }
    } catch(e) {}
  }, []);
  ...
  async function handleScan() {
    ...
    setError("");
    try {
      setProgress(10); setProgressLabel("Fetching NS records...");
      const ns = await queryNS(d);

      setProgress(25); setProgressLabel("Fetching A records...");
      const [Ares, AAAAres] = await Promise.all([queryAll(d, "A"), queryAll(d, "AAAA")]);

      setProgress(45); setProgressLabel("Fetching MX records...");
      const MXres = await queryAll(d, "MX");

      setProgress(60); setProgressLabel("Fetching TXT / SPF...");
      ...
      setProgress(100);
      setResult({ domain: d, ns, records, flags, prop, critical, warnings, total, spfVal, dmarcVal });
    } catch(e) {
      ...
      <div className="app">
        ...
        <div className="search-wrap">
          <div className="search-row">
            ...
            />
            <button id="btn-scan-main" className="btn-scan" onClick={handleScan} disabled={!domain.trim() || scanning}>
        ...
        {result && (
          <div className="fade-up">
            ...
            {/* NEW: NS Records section */}
            <CollapsibleSection title="Nameservers" count={result.ns ? result.ns.length : 0} defaultOpen={true}>
              <NSSection ns={result.ns} />
            </CollapsibleSection>
            ...
            {/* NEW: SMTP section */}
            <CollapsibleSection title="SMTP Host Check" count={result.records.MX ? result.records.MX.length : 0} defaultOpen={false}>
              <SMTPSection mxRecords={result.records.MX} domain={result.domain} />
            </CollapsibleSection>

            {/* NEW: DMARC RUA section */}
            <CollapsibleSection
              title="DMARC Aggregate Reports (rua)"
              badge={result.dmarcVal && parseDMARCRua(result.dmarcVal).length === 0
                ? <span className="badge badge-warn">No rua</span>
                : null}
              defaultOpen={false}
            >
              <DMARCRuaSection dmarcVal={result.dmarcVal} domain={result.domain} />
            </CollapsibleSection>
        ...
        {!result && !scanning && (
          <div className="empty-state">
            ...
            <div className="empty-sub">Checks A, AAAA, MX, NS, SPF, DMARC and more — flags misconfigurations and validates email deliverability.</div>
          </div>
        )}
